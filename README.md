...
Open a video, choose the **Spoken language** when it is known, and click **Generate subtitles**. The current native multilingual ASR path does not claim automatic language identification: leaving the source language unspecified keeps the generated track language as `und`, while translation requires an explicit source language. Native WhisperX resolves the models it needs and downloads missing ones automatically: the existing `small` ASR model, the configured alignment model, and only the requested translation language route when translation is selected. Model resolution, downloading and loading appear as distinct status messages. Downloads may be large and require internet access, a writable cache and free disk space. Subsequent runs reuse cached model artifacts; simply opening/editing subtitles does not download AI models.

The model cache defaults to the OS cache directory under `subtitle-merger/models`; the generation panel displays its exact location. Set `SUBTITLE_MODEL_CACHE_DIR` to override it. ASR, alignment and translation use this same root. Downloads and inference remain owned by Native WhisperX; the editor does not implement a second model cache or downloader. `SUBTITLE_MODEL_CACHE_ONLY=1` requires all AI models to resolve from that application cache and disables model downloads, which is useful for offline/cache validation. Failure messages retain the native cause, release the generation controls for retry, and preserve source subtitles if only translation fails. If native forced word alignment reaches the known content-specific `CTC path is impossible` mismatch after ASR succeeds, Subtitle Merger retries the transcription without word alignment instead of discarding the usable transcript; the completed job message makes that degradation visible.
...
The browser transcription path is fully browser-local without rereading the whole video into one buffer. Rust/WASM owns container demux planning and requests bounded `File.slice()` ranges; the dedicated browser worker currently emits MP4/MOV AAC or Matroska/WebM Opus packets to WebCodecs `AudioDecoder`. Decoded `AudioData` is passed to `audio-analysis`, which owns continuous downmix/resampling to 16 kHz mono, bounded Whisper backpressure, the reviewed Tiny/Base/Small model catalog, browser model caching, WebGPU pipeline lifecycle, and inference. Range acquisition pauses while decode/inference drains, so slow inference cannot create the old real-time capture backlog. There is no application-server, Python, CPU-inference, whole-video `arrayBuffer()`, or MediaStream fallback on the Pages surface; unsupported containers/codecs and owning-layer failures are reported explicitly.
