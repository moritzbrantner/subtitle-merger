...
The browser transcription adapter exposes the reviewed `audio-analysis` WebGPU model catalog directly: Whisper Tiny is the default, with Whisper Base and Whisper Small available as higher-resource choices. The selected model ID is passed back to `audio-analysis`, which owns validation, model loading, browser asset caching, WebGPU pipeline eviction, decoding/resampling, inference, and timed output; Subtitle Merger does not maintain a second model list or runtime. Model assets are downloaded on first use and reused from the browser cache afterward, while superseded idle WebGPU pipelines are disposed instead of accumulating as the selection changes. The selected Reference Video remains attached to its file input while it is in use, then the browser decodes the finite local file and transcribes it directly; there is no live MediaStream fallback. If the browser reports a stale or unavailable file handle, re-select the Reference Video and retry. Decode, WebGPU, and memory-allocation failures keep their original diagnostic so large or unsupported media can be moved to the host-native workflow instead. There is no application-server, Python, CPU-inference, or streaming fallback on the Pages surface. Alignment, diarization, translation, and broader Native WhisperX Workflow Composition are intentionally not approximated there.
