...
The browser transcription adapter uses the small `onnx-community/whisper-tiny` model with WebGPU. Model assets are downloaded on first use and reused from the browser cache afterward. The selected Reference Video remains attached to its file input while it is in use, then the browser decodes the finite local file and transcribes it directly through `audio-analysis`; Subtitle Merger does not switch to a live MediaStream capture pipeline. If the browser reports a stale or unavailable file handle, re-select the Reference Video and retry. Decode, WebGPU, and memory-allocation failures keep their original diagnostic so large or unsupported media can be moved to the host-native workflow instead. There is no application-server, Python, CPU-inference, or streaming fallback on the Pages surface. Alignment, diarization, translation, and broader Native WhisperX Workflow Composition are intentionally not approximated there.
