...
- never requires a whole-video `File.arrayBuffer()` for browser transcription; a Rust/WASM audio-demux session requests bounded ranges from the selected file, currently producing MP4/MOV AAC or Matroska/WebM Opus packets;
- decodes those packets with WebCodecs `AudioDecoder` in a dedicated module worker, then passes decoded `AudioData` to the reviewed `audio-analysis` adapter, which owns continuous downmix/resampling to 16 kHz mono and bounded Whisper backpressure;
- pauses range acquisition whenever decode/transcription work is being drained, so inference can run slower than media duration without an ever-growing capture backlog;
- exposes the reviewed upstream WebGPU Whisper model catalog to the user, defaults to Whisper Tiny, and passes the selected model ID back to `audio-analysis`; the upstream adapter validates IDs and disposes superseded idle WebGPU pipelines;
...
Reference-video **inspection and embedded-subtitle extraction** memory use is no longer proportional to the full media file. Large video payloads remain browser-local `File` data and are copied into WebAssembly only for bounded metadata or subtitle ranges requested by Rust. Container scanning can therefore handle multi-gigabyte media without requiring a same-sized WebAssembly allocation, subject to the explicit per-range bounds above. Browser transcription follows the same bounded-source principle: Rust/WASM reads only container metadata and planned encoded-audio ranges, WebCodecs releases decoded frames after they are resampled, and the upstream Whisper session bounds queued 16 kHz PCM. It does not materialize the entire video or tie acquisition to real-time playback.
...
Browser generation is intentionally narrower than the host-native generation workflow: it requires WebGPU, WebCodecs, and a supported browser-demux combination (currently MP4/MOV AAC or Matroska/WebM Opus). It exposes only the curated Whisper Tiny/Base/Small catalog owned by the reviewed upstream browser adapter, downloads/caches the selected model there, and fails closed rather than accepting arbitrary remote models or falling back to a server, Python, CPU inference, whole-file decode, or live MediaStream path. File-range, demux, codec, model, WebGPU, and memory failures retain their owning-layer diagnostics. Static build validation proves the pinned integration contract; Chromium acceptance separately proves real WASM range demux plus WebCodecs decode without downloading AI weights.
