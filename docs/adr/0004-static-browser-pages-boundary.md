...
Browser generation is intentionally narrower than the host-native generation workflow: it requires WebGPU and browser-decodable media, downloads/caches its model through the reviewed upstream browser adapter, and fails closed rather than falling back to a server, Python, or CPU inference path. Small inputs retain the faster direct browser decode. Large or otherwise unreadable whole-file inputs use media playback as the acquisition clock, so that bounded fallback cannot complete faster than the media duration; the tradeoff is deterministic memory behavior and no full-file reread. Static build validation proves the pinned integration contract but is not evidence that a particular deployed browser/GPU completed real inference.
