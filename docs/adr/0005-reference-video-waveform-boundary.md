...
Waveform generation is presentation-only and non-blocking for video/subtitle loading. The accepted video/subtitle session commits before waveform analysis completes; a late waveform is attached to the current document without rebuilding subtitle state. A failed waveform analysis must not replace or invalidate the accepted session, and an analysis result from a superseded load attempt must never commit.
